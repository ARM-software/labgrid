...
class _SessionManager:
    ...
    def get_session_objs(self, session):
        self._assert_session(session)
        return {k: copy(v) for k, v in self._sessions[session].items()}
...
class Coordinator(labgrid_coordinator_pb2_grpc.CoordinatorServicer):
            ...
            try:
                async for in_msg in request_iterator:
                    ...
                    if kind == "sync":
                        ...
                        client_session = self._session_name(peer, in_msg.startup.session)
                        session = self.clients[peer] = ClientSession(
                            self, peer, name, out_msg_queue, version, client_session
                        )
                        if session.session:
                            self.session_manager.add_session(client_session)
    ...
    async def AcquirePlace(self, request, context):
        ...
        if place.reservation:
            ...
            if res.session and res.session != self._session_name(peer, request.session):
                await context.abort(
                    grpc.StatusCode.PERMISSION_DENIED,
                    f'Place session "{request.session}" is not the same as the reservation session "{res.session}"',
                )
        ...
        place.session = self._session_name(peer, request.session)
        if place.session:
    ...
    async def CreateReservation(self, request: labgrid_coordinator_pb2.CreateReservationRequest, context):
        ...
        res = Reservation(
            owner=owner, prio=request.prio, filters=fltrs, session=self._session_name(peer, request.session)
        )
        if request.session:
    ...
    async def _release_session(self, client_session):
        ...
        objs = self.session_manager.get_session_objs(session)
        await self.lock.acquire()
        for place in objs["places"]:
            await self._release_place(place)
        for reservation in objs["reservations"]:
            self._release_reservation(reservation.token)
        self.lock.release()

        self.session_manager.remove_session(session)

    def _session_name(self, peer, name):
        if not name:
            return ""
